...
export type TimelineItemType = 'work' | 'education' | 'award' | 'milestone'
...
const typeColorMap: Record<TimelineItemType, { dot: string; icon: string; badge: string }> = {
  work:      { dot: 'bg-violet-500 shadow-violet-500/50', icon: 'text-violet-400', badge: 'bg-violet-500/15 text-violet-300 border-violet-500/30' },
...
<template>
  ...
  >
    ...
    <div class="text-center mb-16 px-4">
      ...
      <p class="mt-4 max-w-xl mx-auto text-slate-400 text-base sm:text-lg leading-relaxed">
        {{ props.subtitle }}
      </p>
    </div>
    ...
    <div class="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div
        class="absolute top-0 bottom-0 left-[calc(1.25rem-1px)] sm:left-[calc(1.5rem-1px)] lg:left-1/2 lg:-translate-x-px w-px bg-gradient-to-b from-transparent via-slate-700 to-transparent"
        aria-hidden="true"
      />
      ...
      <ol class="relative space-y-0" aria-label="Career timeline">
        ...
        >
          ...
          >
            <article
              class="group relative bg-slate-900 border border-slate-800 rounded-2xl p-5 sm:p-6 hover:border-slate-700 hover:bg-slate-800/80 transition-all duration-300 hover:shadow-xl hover:shadow-black/30"
            ...
            >
              ...
              <div class="flex items-start justify-between gap-3 mb-3">
                <div class="flex items-center gap-3">
                  ...
                  <div
                    v-if="item.logoUrl"
                    class="w-10 h-10 rounded-xl bg-slate-800 border border-slate-700 overflow-hidden flex-shrink-0"
                  >
                    <img
                      :src="item.logoUrl"
              ...
              <!-- Title + org -->
              <h3 class="text-base font-bold text-slate-100 group-hover:text-white transition-colors leading-snug">
                {{ item.title }}
              </h3>
              ...
              <div class="flex flex-wrap items-center gap-x-4 gap-y-1 mt-2 text-xs text-slate-500">
                <span class="flex items-center gap-1">
                  ...
                  <time>{{ formatDateRange(item.dateStart, item.dateEnd, item.current) }}</time>
                  <!-- Current badge -->
                  <span
                    v-if="item.current"
              ...
              >
                <li
                  ...
                  :key="tag"
                  class="inline-flex items-center px-2 py-0.5 rounded-md bg-slate-800 border border-slate-700 text-[11px] font-medium text-slate-300"
                >
                  {{ tag }}
                </li>
              </ul>
          ...
          >
            <div
              :class="[
                'w-4 h-4 rounded-full ring-4 ring-[#09090f] shadow-lg transition-all duration-700 delay-300',
